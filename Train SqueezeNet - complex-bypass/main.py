...
def test():
    ...
    for idx, (data, target) in enumerate(test_loader):
        ...
        test_correct += pred.reshape(-1).eq(target.data).cpu().sum()
...
def val():
    ...
    for idx, (data, target) in enumerate(test_loader):
        ...
        correct += pred.reshape(-1).eq(target.data).cpu().sum()
        total_val += len(target.data)
        # print("pred len: ", len(pred))
        # print("target.data len: ", len(target.data))
    ...
    print("predicted {} out of {}".format(correct, total_val))
    val_accuracy = correct.double() / total_val * 100
    ...
    if val_accuracy > best_accuracy:
        ...
        # save the model
        print("best_accuracy was", best_accuracy, "\n new best is ", val_accuracy)
...
if __name__ == '__main__':
    if not args.want_to_test:
        ...
        for i in range(1,args.epoch+1):
            train_i = train(i)
            train_acc.append(train_i)
            val_i = val()
            val_acc.append(val_i)
            # rows = zip(train_acc.insert(0, "train_acc"), val_acc.insert(0, "val_acc"))
            import os, csv
            if not os.path.isfile("acc.csv"):
